...
{
    // 1. 对扑克牌进行解析: 1张的, 2张, 3张, 4张有多少种
    classify(cards);
    // 2. 对牌型进行分类
    judgeCardType();
}

PlayHand::PlayHand(HandType type, CardPoint pt, int extra)
{
    m_type = type;
    m_pt = pt;
    m_extra = extra;
...
void PlayHand::classify(Cards &cards)
{
    ...
    int cardRecord[(int)CardPoint::Card_End];
    memset(cardRecord, 0, sizeof(int) * (int)CardPoint::Card_End);

    for(int i=0; i<list.size(); ++i)
    {
        Card c = list.at(i);
        cardRecord[(int)c.point()] ++;
    }
    ...
    for(int i=0; i<(int)CardPoint::Card_End; ++i)
    {
        if(cardRecord[i] == 1)
        {
...
bool PlayHand::isPlane()
{
    if(m_oneCard.isEmpty() && m_twoCard.isEmpty() && m_threeCard.size() == 2 && m_fourCard.isEmpty())
    {
        ...
        if((int)m_threeCard[1] - (int)m_threeCard[0] == 1 && m_threeCard[1] < CardPoint::Card_2)
        {
            return true;
        }
    }
...
bool PlayHand::isPlaneTwoSingle()
{
    if(m_oneCard.size() == 2 && m_twoCard.isEmpty() && m_threeCard.size() == 2 && m_fourCard.isEmpty())
    {
        ...
        if((int)m_threeCard[1] - (int)m_threeCard[0] == 1 && m_threeCard[1] < CardPoint::Card_2 &&
            (m_oneCard[0] != CardPoint::Card_SJ || m_oneCard[1] != CardPoint::Card_BJ))
        {
            return true;
        }
    }
...
bool PlayHand::isSeqPair()
{
    if(m_oneCard.isEmpty() && m_twoCard.size() >= 3 && m_threeCard.isEmpty() && m_fourCard.isEmpty())
    {
        ...
        if((int)m_twoCard.last() - (int)m_twoCard.first() == (m_twoCard.size() -1) &&
            m_twoCard.first() >= CardPoint::Card_3 && m_twoCard.last() < CardPoint::Card_2)
        {
            return true;
        }
    }
...
bool PlayHand::isSeqSingle()
{
    if(m_oneCard.size() >= 5 && m_twoCard.isEmpty() && m_threeCard.isEmpty() && m_fourCard.isEmpty())
    {
        ...
        if((int)m_oneCard.last() - (int)m_oneCard.first() == (m_oneCard.size() -1) &&
            m_oneCard.first() >= CardPoint::Card_3 && m_oneCard.last() < CardPoint::Card_2)
        {
            return true;
        }
    }
...
bool PlayHand::isBombTwoSingle()
{
    ...
    {
        std::sort(m_oneCard.begin(), m_oneCard.end());
        if(m_oneCard.first() != CardPoint::Card_SJ || m_oneCard.last() != CardPoint::Card_BJ)
        {
            return true;
        }
    }
...
bool PlayHand::isBombJokersSingle()
{
    ...
    {
        std::sort(m_oneCard.begin(), m_oneCard.end());
        if(m_oneCard[1] == CardPoint::Card_SJ && m_oneCard[2] == CardPoint::Card_BJ)
        {
            return true;
        }
    }
...
bool PlayHand::isBombJokersPair()
{
    ...
    {
        std::sort(m_oneCard.begin(), m_oneCard.end());
        if(m_oneCard[0] == CardPoint::Card_SJ && m_oneCard[1] == CardPoint::Card_BJ)
        {
            return true;
        }
    }
...
bool PlayHand::isBombJokersTwoSingle()
{
    ...
    {
        std::sort(m_oneCard.begin(), m_oneCard.end());
        if(m_oneCard[2] == CardPoint::Card_SJ && m_oneCard[3] == CardPoint::Card_BJ)
        {
            return true;
        }
    }
