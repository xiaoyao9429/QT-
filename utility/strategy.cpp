...
Cards Strategy::firstPlay()
{
    ...
    // 有没有炸弹
    QVector<Cards> bombArray = findCardType(PlayHand(PlayHand::Hand_Bomb, CardPoint::Card_Begin, 0), false);
    backup.remove(bombArray);

    // 有没有飞机
    QVector<Cards> planeArray = Strategy(m_player, backup).findCardType(PlayHand(PlayHand::Hand_Plane, CardPoint::Card_Begin, 0), false);
    ...
    // 有没有三张点数相同的牌
    QVector<Cards> seqTripleArray = Strategy(m_player, backup).findCardType(PlayHand(PlayHand::Hand_Triple, CardPoint::Card_Begin, 0), false);
    ...
    // 有没有连对
    QVector<Cards> seqPairArray = Strategy(m_player, backup).findCardType(PlayHand(PlayHand::Hand_Seq_Pair, CardPoint::Card_Begin, 0), false);
    ...
    if(hasPlane)
    {
        ...
        for(CardPoint point = CardPoint::Card_3; point <= CardPoint::Card_10; point = CardPoint((int)point + 1))
        {
    ...
    if(hasTriple)
    {
        if(PlayHand(seqTripleArray[0]).getCardPoint() < CardPoint::Card_A)
        {
            for(CardPoint point = CardPoint::Card_3; point <= CardPoint::Card_A; point=(CardPoint)((int)point+1))
            {
    ...
    {
        for(CardPoint point = CardPoint((int)CardPoint::Card_End-1);
             point >= CardPoint::Card_3; point = CardPoint((int)point-1))
        {
            int pointCount = backup.pointCount(point);
            if(pointCount == 1)
            {
    ...
    {
        for(CardPoint point =  CardPoint::Card_3;
             point < CardPoint::Card_End; point =(CardPoint)((int)point+1))
        {
            int pointCount = backup.pointCount(point);
            if(pointCount == 1)
            {
...
Cards Strategy::getGreaterCards(PlayHand type)
{
    ...
    if(pendPlayer != nullptr && pendPlayer->role() != m_player->role() && pendPlayer->cards().cardCount() <= 3)
    {
        ...
        // 搜索当前玩家手中有没有王炸
        Cards sj = findSamePointCards(CardPoint::Card_SJ, 1);
        Cards bj = findSamePointCards(CardPoint::Card_BJ, 1);
...
bool Strategy::whetherToBeat(Cards &cs)
{
    ...
    if(m_player->role() == pendPlayer->role())
    {
        ...
        CardPoint basePoint = PlayHand(cs).getCardPoint();
        if(basePoint == CardPoint::Card_2 || basePoint == CardPoint::Card_SJ || basePoint == CardPoint::Card_BJ)
        {
            return false;
        }
    }
    else
    {
        ...
        if((myHand.getHandType() == PlayHand::Hand_Triple_Single || myHand.getHandType() == PlayHand::Hand_Triple_Pair)
                && myHand.getCardPoint() == CardPoint::Card_2)
        {
            return false;
        }

        // 如果cs是对2, 并且出牌玩家手中的牌数量大于等于10 && 自己的牌的数量大于等于5, 暂时放弃出牌
        if(myHand.getHandType() == PlayHand::Hand_Pair && myHand.getCardPoint() == CardPoint::Card_2
...
Cards Strategy::findSamePointCards(CardPoint point, int count)
{
    ...
    int findCount = 0;
    Cards findCards;
    for(int suit = (int)CardSuit::Suit_Begin+1; suit < (int)CardSuit::Suit_End; suit++)
    {
        Card card;
        card.setPoint(point);
        card.setSuit((CardSuit)suit);
        if(m_cards.contains(card))
        {
            findCount ++;
            findCards.add(card);
...
{
    Cards rangeCards;
    for(CardPoint point = begin; point < end; point = (CardPoint)((int)point+1))
    {
...
QVector<Cards> Strategy::findCardType(PlayHand hand, bool beat)
{
    ...
    // 确定起始点数
    CardPoint beginPoint = beat ? CardPoint((int)point + 1) : CardPoint::Card_3;
...
{
    // 1. 得到所有顺子的组合
    QVector<Cards> allSeq = Strategy(m_player, cards).findCardType(PlayHand(PlayHand::Hand_Seq_Single, CardPoint::Card_Begin, 0), false);
    if(allSeq.isEmpty())
    {
        // 结束递归,将满足条件的顺子传递给调用者
...
QVector<Cards> Strategy::getCards(CardPoint point, int number)
{
    ...
    for(CardPoint pt=point; pt < CardPoint::Card_End; pt = (CardPoint)((int)pt + 1))
    {
        // 目的是尽量不拆分别的牌型
...
QVector<Cards> Strategy::getTripleSingleOrPair(CardPoint begin, PlayHand::HandType type)
{
    // 找到点数相同的三张牌
    ...
    if(!findCardArray.isEmpty())
    {
        ...
        QVector<Cards> cardsArray = st.findCardType(PlayHand(type, CardPoint::Card_Begin, 0), false);
        if(!cardsArray.isEmpty())
        {
            // 将找到的牌和三张点数相同的牌进行组合
            for(int i=0; i<findCardArray.size(); ++i)
            {
                findCardArray[i].add(cardsArray.at(0));//拼一种就行
            }
        }
        else
        {
...
QVector<Cards> Strategy::getPlane(CardPoint begin)
{
    QVector<Cards> findCardArray;
    for(CardPoint point = begin; point <= CardPoint::Card_K; point = (CardPoint)((int)point+1))
    {
        // 根据点数和数量进行搜索
        Cards prevCards = findSamePointCards(point, 3);
        Cards nextCards = findSamePointCards((CardPoint)((int)point+1), 3);
...
QVector<Cards> Strategy::getPlane2SingleOr2Pair(CardPoint begin, PlayHand::HandType type)
{
    // 找飞机
...
QVector<Cards> Strategy::getSepPairOrSeqSingle(CardInfo &info)
{
    ...
    {
        // 最少3个, 最大A
        for(CardPoint point = info.begin; point <= info.end; point = (CardPoint)((int)point+1))
        {
            bool found = true;
            Cards seqCards;
            ...
            {
                // 基于点数和数量进行牌的搜索
                Cards cards = findSamePointCards((CardPoint)((int)point + i), info.number);
                if(cards.isEmpty() || ((int)point + info.extra >= (int)CardPoint::Card_2))
                {
                    found = false;
                    seqCards.clear();
                    break;
    ...
    else
    {
        for(CardPoint point = info.begin; point <= info.end; point = (CardPoint)((int)point+1))
        {
            ...
            {
                // 新的起始点数
                CardPoint followedPoint = CardPoint((int)point + followed);
                // 判断是否超出了上限
                if(followedPoint >= CardPoint::Card_2)
                {
                    break;
                }
...
Cards Strategy::getBaseSeqPair(CardPoint point)
{
    Cards cards0 = findSamePointCards(point, 2);
    Cards cards1 = findSamePointCards((CardPoint)((int)point+1), 2);
    Cards cards2 = findSamePointCards((CardPoint)((int)point+2), 2);
    Cards baseSeq;
...
Cards Strategy::getBaseSeqSingle(CardPoint point)
{
    Cards cards0 = findSamePointCards(point, 1);
    Cards cards1 = findSamePointCards((CardPoint)((int)point+1), 1);
    Cards cards2 = findSamePointCards((CardPoint)((int)point+2), 1);
    Cards cards3 = findSamePointCards((CardPoint)((int)point+3), 1);
    Cards cards4 = findSamePointCards((CardPoint)((int)point+4), 1);
    Cards baseSeq;
...
QVector<Cards> Strategy::getBomb(CardPoint begin)
{
    QVector<Cards> findcardsArray;
    for(CardPoint point = begin; point < CardPoint::Card_End; point = (CardPoint)((int)point + 1))
    {
