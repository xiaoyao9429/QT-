...
public:
    ...
    // 6. 找出所有点数数量只为count的牌 ==> 得到一个多张扑克牌数组
...
private:
    using function = Cards (Strategy::*)(CardPoint point);
    struct CardInfo
    {
        CardPoint begin;
        CardPoint end;
    ...
    QVector<Cards> getTripleSingleOrPair(CardPoint begin, PlayHand::HandType type);
    QVector<Cards> getPlane(CardPoint begin);
    QVector<Cards> getPlane2SingleOr2Pair(CardPoint begin, PlayHand::HandType type);
    QVector<Cards> getSepPairOrSeqSingle(CardInfo &info);
    Cards getBaseSeqPair(CardPoint point);
    Cards getBaseSeqSingle(CardPoint point);
    QVector<Cards> getBomb(CardPoint begin);
